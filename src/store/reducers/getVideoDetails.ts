...
export const getVideoDetails = createAsyncThunk(
    "utubeApp/videoDetails",
    async (id: string) => {
        const {
            data: { items },
        } = await axios.get(
            `${UTUBE_API_URL}/videos?key=${API_KEY}&part=snippet,statistics&type=video&id=${id}`
        );
...
const parseData = async (item: {
    ...
    const {
        ...
        `${UTUBE_API_URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`
    );
