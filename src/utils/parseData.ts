...
import { UTUBE_API_URL } from "./constants";
...
export const parseData = async (items: any[]) => {
    try {
        ...
        const {
            ...
            `${UTUBE_API_URL}/channels?part=snippet,contentDetails&id=${channelIds.join(
                ","
            )}&key=${API_KEY}`
        );
        ...
        const {
            ...
            `${UTUBE_API_URL}/videos?part=contentDetails,statistics&id=${videoIds.join(
                ","
            )}&key=${API_KEY}`
        );
        ...
        items.forEach(
            (
                ...
                if (channelImage)
                    console.log(videosData[index]?.contentDetails?.duration);
                parsedData.push({
                    ...
                    videoDuration: videosData[index]?.contentDetails?.duration !== undefined ? parseVideoDuration(
